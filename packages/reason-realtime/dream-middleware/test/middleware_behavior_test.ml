...
Alcotest.test_case "detach unsubscribes active channel" `Quick (fun () ->
  ...
  let wrapped = Option.value ~default:(fun ~channel:_ s -> s) (Some (fun ~channel s ->
    Printf.sprintf "{\"type\":\"patch\",\"channel\":\"%s\",\"data\":%s}" channel s)) in
  let* _ = handler ~wrap:wrapped "test-payload" in
  Lwt.return_unit)
  in
  if String.contains (List.hd (List.rev !sent)) 'c' then ()
  else Alcotest.fail "Expected patch to include channel field");
  ...
  let handle_mutation broadcast _request ~db:_ ~action_id:_ ~mutation_name:_ _action =
    broadcast ~channel:"c" "test-mutation-result";
    Lwt.return (Mutation_result.Ack (Ok ()))
  in
  let runtime = make_runtime ~handle_mutation adapter in
  let sent = ref [] in
  ...
  ~subscribe:(fun channel -> Lwt.return_some channel))
  in
  let ack_json = List.hd (List.rev !sent) in
  if String.contains ack_json 'c' then ()
  else Alcotest.fail "Expected ack to include channel field");
  ...
  | None ->
    ...
    failwith "Failed to construct websocket for detach test"
  in
  Hashtbl.replace runtime.Middleware.channels "multi-room"
  [{ websocket; current_subscriptions = ["room-a"; "room-b"]; pending_sends = []; send_in_progress = false }];
  let _ = Lwt_main.run (Middleware.detach_websocket runtime websocket) in
