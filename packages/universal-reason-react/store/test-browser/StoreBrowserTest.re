...
let run = () => {
  ...
  |> then_(browser =>
       ...
       |> then_(page =>
            ...
            |> then_(text =>
                 assertContains(~label="Hydrated seeded state", ~expected="Learn ReasonML", text)
                 |> then_(_ => assertContains(~label="Initial stats", ~expected="0 of 3 completed", text))
               )
            ...
            |> then_(_ => bodyText(page))
            |> then_(text => assertContains(~label="Stats after toggle", ~expected="1 of 4 completed", text))
