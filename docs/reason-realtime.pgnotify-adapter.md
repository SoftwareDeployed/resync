...
> Note: the adapter only delivers notifications that Postgres emits. For realtime table sync, you still need to generate and apply trigger SQL such as `generated/realtime.sql` for your schema.
...
Triggers are auto-generated by the `realtime-schema` package from annotated SQL files. See `REALTIME_QUERY_REFACTOR.md` for the annotation syntax. The generated triggers produce payloads in the format below.

For manual trigger setup:
...
The adapter forwards all notification payloads from PostgreSQL triggers; the
application-level `decodePatch` function on the client determines which tables
and actions to handle.
