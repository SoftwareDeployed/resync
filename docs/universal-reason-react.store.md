...
The runtime export also includes store-scoped hooks. `useStreaming()` returns the typed streaming state reactively; local stores return `unit`, and synced stores update after typed stream events plus confirmed patch reconciliation. `useQuery((module Query), params, ~skip=false, ())` subscribes through the shared query cache and returns the store with loaded rows applied; pass `~skip=true` to preserve hook order while avoiding SSR registration and client subscription. `useQueryOption((module Query), paramsOpt, ())` is the Reason-friendly conditional form: `Some(params)` subscribes and `None` skips without requiring placeholder params. `useQueryResult((module Query), params, ~skip=false, ())` uses the same subscription path and returns the low-level `{data, loading, error}` query result when a component needs the rows directly. When skipped, `useQueryResult` returns `{data: Loading, loading: false, error: None}`. `useQueryResultOption((module Query), paramsOpt, ())` is the low-level result form of `useQueryOption`. `useIsQueryLoading((module Query), params)` returns the query cache loading state. `useMutation((module Mutation), ())` returns a mutation handle whose `dispatch(params)` and `mutate(params)` fields are aliases that return `Js.Promise.t(unit)`, plus `loading` and `error` fields for the in-flight state and latest rejection message. `useMutationFn((module Mutation), ())` returns the async mutation function directly when a component does not need those state fields. Local stores resolve after local dispatch and reject when validation denies the action or the store is unavailable. Synced stores resolve after server acknowledgement and reject when validation or the server mutation fails. Custom store-scoped mutation modules only need `type params`, `type action`, and `let toAction: params => action`.
