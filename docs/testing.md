...
Today, the repo supports **suite coverage**, **scenario coverage**, and **line coverage** directly:
...
4. **Line coverage** — `bisect_ppx` is installed and instrumented on library dune files (store native, pgnotify-adapter src). Running instrumented tests produces coverage reports on the actual library source code.
...
`bisect_ppx` instrumentation is configured on the **library** dune files, not the test executables. This means tests exercise the actual library code, and coverage reports show source files from `packages/`, not the tests themselves.

### 1. Run instrumented tests

Set `BISECT_FILE` so `bisect_ppx` writes `.coverage` files to a known path, then build and run with `--instrument-with bisect_ppx`:
...
> **Note:** `dream-middleware` is not instrumented because `bisect_ppx` currently crashes on one of its source files. Coverage for that library is not available until the PPX is fixed upstream.
