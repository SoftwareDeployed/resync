...
- **`subscribeStatus`** / **`unsubscribeStatus`** — Subscribe to or unsubscribe from status changes. `subscribeStatus` returns a typed `status_listener_id`. Listeners are notified on every lifecycle change (ready, idle, connection, pending counts). Implemented in `StoreRuntimeLifecycle.re`.
