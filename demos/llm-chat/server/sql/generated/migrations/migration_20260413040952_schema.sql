...
CREATE TABLE IF NOT EXISTS _resync_actions_create_thread (
...
CREATE TABLE IF NOT EXISTS _resync_actions_add_message (
  action_id uuid PRIMARY KEY,
  status text NOT NULL CHECK (status IN ('ok', 'failed')),
  processed_at timestamptz NOT NULL DEFAULT NOW(),
  error_message text
...
CREATE TABLE IF NOT EXISTS _resync_actions_update_message_content (
  action_id uuid PRIMARY KEY,
  status text NOT NULL CHECK (status IN ('ok', 'failed')),
  processed_at timestamptz NOT NULL DEFAULT NOW(),
  error_message text
...
INSERT INTO schema_migrations (version) VALUES ("20260413040952") ON CONFLICT (version) DO NOTHING;
