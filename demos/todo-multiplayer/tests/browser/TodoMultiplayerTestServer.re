...
let rec waitForReady = attemptsLeft =>
  if (attemptsLeft <= 0) {
    reject(Failure("Timed out waiting for todo-multiplayer browser test server"));
  } else {
    fetchUrl(baseUrl)
    |> then_(response =>
...
let start = () => {
  try {
    execFileSync(
      "dune",
      [|"build", "demos/todo-multiplayer/ui/src/.build_stamp", "demos/todo-multiplayer/server/src/server.exe"|],
  ...
  | _ => reject(Failure("Failed to build todo-multiplayer browser test server artifacts"))
  };
};
