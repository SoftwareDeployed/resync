module Websocket = struct
  ...
  module Parser = struct
    ...
    let test_completed_payload_advances_queued_frame () =
     let frames_handled = ref 0 in
     let payloads = ref [] in
     let websocket_handler wsd =
       let frame ~opcode ~is_fin:_ ~len:_ payload =
         match opcode with
     ...
     let read = Server_connection.read t bs ~off:0 ~len in
     Alcotest.(check int) "Reads both frames" len read;
     Alcotest.(check int) "Only current frame handled before payload drain" 1 !frames_handled;
     begin match !payloads with
     | [payload] -> drain_payload payload
     | _ -> Alcotest.fail "Expected one queued payload"
     end;
     ignore @@ Server_connection.next_read_operation t;
     Alcotest.(check int)
       "Queued frame handled without another socket read"
       2 !frames_handled;
    ;;
    ...
    let tests =
      ...
      ; "completed payload advances queued frame", `Quick, test_completed_payload_advances_queued_frame
      ]
  end
